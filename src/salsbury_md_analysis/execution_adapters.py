...
def _project_cached_modules(
    ...
    if module_id == "reactive_path_ensembles":
        return {
            candidate for candidate in (
                "clustering_kmeans", "markov_state_models",
            ) if candidate in definitions
        }
    if module_id == "interaction_fingerprints":
        definition = definitions.get(module_id)
        sources = (
            definition.get("source_modules")
            if isinstance(definition, dict) else None
        )
        return (
            {str(source) for source in sources if str(source) in definitions}
    ...
    for task in tasks:
        ...
        if task is cache_task:
            ...
            for requirement in required_modules:
                ...
                # are complete. If the producer fails, the consumer unsets the
                # cache variable and recomputes from its project inputs.
