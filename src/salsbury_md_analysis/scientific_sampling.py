...
_PROFILES.extend([
    ...
    _inherited("perturbation_response_dynamics", "common_pca", "linear_response_profile", per_replica=250, per_system=1_000, rationale="DFI and DCI inherit the common-basis covariance sample and are static ensemble estimators"),
    _inherited("trajectory_reweighting", "common_pca", "weighted_static_ensemble", per_replica=250, per_system=1_000, rationale="reweighted ensemble estimates require broad frame-aligned common-basis coverage and an effective-sample-size diagnostic"),
    ...
    _inherited("allosteric_pathways", "dccm", "contact_weighted_path_network", per_replica=250, per_system=1_000, rationale="pathway scores inherit the DCCM frame set and contact-occupancy support"),
    _profile("energetic_network_embeddings", "static_energy_network", 100, 500, rationale="energy-network embeddings require distributed configurations and declared force-field parameters"),
    _profile("multivalent_molecular_bridges", "bridge_occupancy_and_persistence", 200, 1_000, maximum_spacing_ns=1.0, contiguous=True, temporal_rule="segment_contiguous_bridge_events", events=20, rationale="bridge occupancies are static, while reported persistence runs require segment-safe ordered observations"),
    _profile("hydration_density_channels", "aligned_density_field", 100, 500, rationale="aligned solvent and ion density fields require distributed configurations but no universal time-gap gate"),
    _profile("ensemble_pocket_dynamics", "pocket_occupancy_and_tracking", 100, 500, maximum_spacing_ns=1.0, contiguous=True, temporal_rule="segment_contiguous_pocket_tracking", events=20, rationale="pocket occupancies need distributed configurations and pocket tracks need ordered segment-safe observations"),
    _profile("interaction_fingerprints", "typed_interaction_occupancy", 200, 1_000, events=20, rationale="typed interaction occupancies require repeated observations across every system"),
    _inherited("spatial_interaction_ensembles", "interaction_fingerprints", "aligned_interaction_point_ensemble", per_replica=100, per_system=500, events=20, rationale="spatial interaction modes inherit chemically typed observations and require populated aligned point clouds"),
    _inherited("interaction_persistence", "interaction_fingerprints", "interaction_event_persistence", per_replica=500, per_system=2_000, maximum_spacing_ns=0.5, contiguous=True, temporal_rule="segment_contiguous_interaction_events", events=50, rationale="interaction lifetimes require ordered, segment-safe observations with explicit censoring"),
    _inherited("helical_mechanics", "nucleic_acid_structure", "helical_parameter_dependence", per_replica=200, per_system=1_000, rationale="helical parameter distributions and static dependence estimates inherit nucleic-acid structural observations"),
    ...
    _inherited("random_feature_koopman", "time_lagged_independent_component_analysis", "nonlinear_time_lagged_basis", per_replica=500, per_system=2_000, maximum_spacing_ns=0.5, contiguous=True, temporal_rule="segment_contiguous_lag_pairs_with_lag_sensitivity", events=100, rationale="random-feature Koopman models require ordered lag pairs and held-out stability checks"),
    ...
    _inherited("reactive_path_ensembles", "clustering_kmeans", "reactive_path_ensemble", per_replica=500, per_system=2_000, maximum_spacing_ns=0.5, contiguous=True, temporal_rule="segment_contiguous_transition_paths", events=100, rationale="reactive-path extraction requires ordered state assignments and enough observed transitions to characterize path families"),
